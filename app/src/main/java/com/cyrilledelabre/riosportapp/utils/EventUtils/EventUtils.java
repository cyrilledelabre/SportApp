...
public class EventUtils {
    ...
    /**
     * Returns a list of {@link DecoratedEvent}s.
     ...
     * @see <code>getProfile</code>
     */
    public static List<DecoratedEvent> getEventsFromRadius()
            throws EventException, IOException {
        if (null == sApiServiceHandler) {
            Log.e(LOG_TAG, "getEvents(): no service handler was built");
            throw new EventException();
        ...
        double latitude = NetworkProvider.getCurrentPosition().latitude;
        double longitude = NetworkProvider.getCurrentPosition().longitude;
        int radius = NetworkProvider.getCurrentRadiusInMeters();

        com.appspot.riosportapp.event.Event.QueryEventsRadius queryEventsRadius = sApiServiceHandler.queryEventsRadius(latitude, longitude, radius);
        EventCollection eventCollection = queryEventsRadius.execute();
